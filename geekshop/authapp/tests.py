...
class TestMainappSmoke(TestCase):
    def setUp(self):
        ...
        self.superuser = ShopUser.objects.create_superuser('django2',
                                                           ...
                                                           'geekbrains')
        self.user = ShopUser.objects.create_user('balabanov',
                                                 'balabanov@geekshop.local',
                                                 'geekbrains')
        self.user_with__first_name = ShopUser.objects.create_user('bodrov',
                                                                  'bodrov@geekshop.local',
                                                                  'geekbrains',
                                                                  first_name='Sergej')
    ...
    def test_user_login(self):
        ...
        # данные пользователя
        self.client.login(username='balabanov', password='geekbrains')
    ...
    def test_user_wrong_register(self):
        ...
        response = self.client.post('/auth/register/', data=new_user_data)
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'register_form', 'age', 'Доступ только 18+!')
